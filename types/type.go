...
type (
	// T is an empty interface, that is `any` type.
	...
	// Predicate is a Function, which produces a bool value. usually used to test whether a value satisfies a condition
	Predicate func(e T) bool
	// UnaryOperator is a Function, which argument and result are same type
	...
	// Comparator is a BiFunction, which two arguments are the same type, and return a int.
	// if left is greater than right, it returns a positive number;
	// if left is less than right, it returns a negative number; if the two input are equal, it returns 0
...
var (
	...
	IntComparator Comparator = func(left T, right T) int {
