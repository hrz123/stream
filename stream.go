...
import (
	...
	"github.com/hrz123/stream/optional"
...
type Stream interface {
	...
	// ToSlice return []T 转为切片
	ToSlice() []types.T
	// ToElementSlice return []X which X is the type of some
	...
	// AllMatch 测试是否所有元素满足条件
	AllMatch(types.Predicate) bool
	// NoneMatch 测试是否没有元素满足条件
	NoneMatch(types.Predicate) bool
	// AnyMatch 测试是否有任意元素满足条件
	...
	// ReduceFrom type of initValue is same as element.  (T, T) -> T
	ReduceFrom(initValue types.T, accumulator types.BinaryOperator) types.T
	// ReduceWith type of initValue is different from element. (R, T) -> R
